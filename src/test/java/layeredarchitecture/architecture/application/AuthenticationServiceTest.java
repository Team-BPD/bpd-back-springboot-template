...
class AuthenticationServiceTest {
    ...
    void shouldGenerateJwtWhenValidClientSystemInfoProvided() {
        // Given
        when(clientSystemRepository.findPasswordById(validAuthDto.getId())).thenReturn(Optional.of("1234"));
        when(jwt.generateToken(validAuthDto.getId())).thenReturn("token");
    ...
    void shouldThrowExceptionWhenClientSystemNotFound() {
        // Given
        when(clientSystemRepository.findPasswordById(validAuthDto.getId())).thenReturn(Optional.empty());

        // When
        CustomException exception = assertThrows(CustomException.class, () -> {
            authenticationService.generatedJwt(validAuthDto);
        });
    ...
    void shouldThrowExceptionWhenIdPasswordNotMatched() {
        // Given
        when(clientSystemRepository.findPasswordById(validAuthDto.getId())).thenReturn(Optional.of("4321"));
